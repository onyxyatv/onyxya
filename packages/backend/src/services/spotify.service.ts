...
class SpotifyService {
  ...
  public static spotifyApiBase: string =
    'https://api.spotify.com/' + this.spotifyApiVersion;
  ...
  static async searchTrackByName(
    searchedName: string,
    ...
    try {
      ...
      if (this.apiKey !== undefined) {
        const res: AxiosResponse<any, any> = await axios.get(
          `${this.spotifyApiBase}/search?q=${searchedName.replaceAll(' ', '%20')}&type=track`,
          {
  ...
  static async getTrackById(trackId: string): Promise<Track | null> {
    try {
      await this.checkApiKey();
      const res: AxiosResponse<any, any> = await axios.get(
        `${this.spotifyApiBase}/tracks/${trackId}`,
        {
          headers: { Authorization: `Bearer ${this.apiKey}` },
        },
      );
